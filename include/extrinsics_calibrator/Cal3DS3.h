...
class Cal3DS3 {
 public:
  enum { dimension = 12 };
  enum { num_coeff = 6 };
  typedef boost::shared_ptr<Cal3DS3> shared_ptr;  ///< shared pointer to calibration object

 protected:
  double fx_, fy_, u0_, v0_;       // focal length,  and principal point
  double p1_, p2_, k_[num_coeff];  // radial distortion coefficients of radtan model
 ...
 public:
  ...
  Cal3DS3(double fx, double fy, double u0, double v0,
          double p1, double p2, const double* k) : fx_(fx), fy_(fy), u0_(u0), v0_(v0), p1_(p1), p2_(p2) {
    for (int i = 0; i < num_coeff; i++) {
      k_[i] = k[i];
    }
  }
  ...
  friend std::ostream& operator<<(std::ostream& os, const Cal3DS3& cal);
