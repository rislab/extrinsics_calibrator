...
namespace gtsam {
/* ************************************************************************* */
Cal3DS3::Cal3DS3(const gtsam::Vector& v) : fx_(v[0]), fy_(v[1]), u0_(v[2]), v0_(v[3]), p1_(v[4]), p2_(v[5]) {
  for (int i = 0; i < 6; i++) {
    k_[i] = v[6 + i];
  }
}
...
std::ostream& operator<<(std::ostream& os, const Cal3DS3& cal) {
  os << "{fx: " << cal.fx() << ", fy: " << cal.fy() << ", px:" << cal.px()
     << ", py:" << cal.py() << ", p1:" << cal.p1_ << ", p2:" << cal.p2_ << ", k1:" << cal.k_[0] << ", k2:" << cal.k_[1] << ", k3:" << cal.k_[2] << ", k4:" << cal.k_[3] << ", k5:" << cal.k_[4] << ", k6:" << cal.k_[5] << "}";
  return os;
}
...
Cal3DS3 Cal3DS3::retract(const Vector& d) const {
  return Cal3DS3(vector() + d);
}

Vector Cal3DS3::localCoordinates(const Cal3DS3& T2) const {
...
D2dcalibration(double x, double y,
  ...
  DR2 << 2 * xy, R + 2 * x2, xdinv * R, xdinv * R2, xdinv * R3, xf * R, xf * R2, xf * R3,  // u
...
gtsam::Point2 Cal3DS3::uncalibrateNoIntrinsics(const gtsam::Point2& p) const {
  GeometricParams gp(p.x(), p.y(), p1_, p2_, k_[0], k_[1], k_[2], k_[3], k_[4], k_[5]);
